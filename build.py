...
# Safety gate: never replace a previously working config with a tiny result.
...
def parse_vless(uri):
    try:
        ...
        if security == "reality":
            ...
            # Mihomo expects Reality short-id to be hexadecimal and to
            # contain a whole number of bytes (0..8 bytes / 0..16 hex chars).
            # Do not let malformed source entries reach the final config.
            raw_short_id = first(query, "sid", "shortId")
            if raw_short_id is not None:
                short_id = dec(raw_short_id).strip()
                if short_id and (
                    len(short_id) > 16
                    or len(short_id) % 2 != 0
...
def check_node(name):
    ...
    for url in TEST_URLS:
        ...
        for _ in range(CHECK_ATTEMPTS):
            delay = probe(name, url)
            if delay is not None:
                successes.append(delay)

        if len(successes) >= REQUIRED_SUCCESSES_PER_URL:
            good_urls += 1
            delays.extend(successes)

        if good_urls >= REQUIRED_GOOD_URLS:
            break

    if good_urls >= REQUIRED_GOOD_URLS and delays:
...
def health_check(nodes):
    ...
    try:
        with open(path, "w", encoding="utf-8") as file:
            yaml.safe_dump(
                make_config(nodes, "127.0.0.1:9090"),
                file,
                allow_unicode=True,
                sort_keys=False,
            )
        ...
        while time.time() < deadline:
            if process.poll() is not None:
                ...
                raise RuntimeError(
                    "Mihomo stopped during health check:\n" + output
                )
            ...
            raise TimeoutError("Mihomo controller did not start")
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            futures = {
                executor.submit(check_node, node["name"]): node
                for node in nodes
            }
            ...
            for future in concurrent.futures.as_completed(futures):
                ...
                try:
                    ...
                    delay = None
                    log(f"[CHECK ERROR] {node['name']}: {error}")

                if delay is not None:
                    node["_delay"] = delay
                    alive.append(node)
                    log(f"[OK] {delay}ms {node['name']}")
                else:
                    log(f"[DEAD] {node['name']}")
...
def main():
    ...
    # Оставляем только проверку структуры VLESS и удаление дублей.
    # Mihomo будет проверять конфигурацию клиента при импорте.
