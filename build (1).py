...
def main():
    ...
    alive.sort(key=lambda node: node.get("_delay", 999999))

    minimum = max(
        MIN_ALIVE_ABSOLUTE,
        math.ceil(len(nodes) * MIN_ALIVE_PERCENT),
    )

    log(
        f"[RESULT] {len(alive)}/{len(nodes)} passed strict check; "
        f"minimum={minimum}"
    )

    if len(alive) < minimum:
        raise RuntimeError(
            "Suspicious health-check result; existing config.yaml preserved"
        )
